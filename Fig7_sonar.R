...
fit            =    glmnet(X, y, family = "binomial", alpha = alpha_elnet,  intercept = FALSE, standardize = FALSE, dfmax = p)
...
eror.plot      =     eror.plot  + ggtitle((sprintf("LO:%0.3f(sec) \n ALO:%0.3f(sec) \n FIT:%.3f(sec)",time.lo,time.alo,time.fit))) 


  if (alpha_elnet == 1) {
    pdf(sprintf("/Users/krad/Dropbox/Fast LOOCV/ALO_JRSSB/figures/sonar_lasso.pdf"))
  } else {
    if (alpha_elnet == 0.5) {
      pdf(sprintf("/Users/krad/Dropbox/Fast LOOCV/ALO_JRSSB/figures/sonar_elnet.pdf"))
    } else {
      pdf(sprintf("/Users/krad/Dropbox/Fast LOOCV/ALO_JRSSB/figures/sonar_ridge.pdf"))
    }
  }
