...
for (i in 1:length(p_)){
  ...
  for (s in 1:MCMCsamples){
    ...
    ptm             =     proc.time()      
    lo              =     cv.glmnet(X, y, alpha = alpha_elnet,  intercept = FALSE, standardize = FALSE, lambda = lambdaS, nfolds = n, type.measure = "deviance")
    ptm             =     proc.time() - ptm
...
if (delta < 1) {
  if (alpha_elnet == 1) {
    pdf("/Users/krad/Dropbox/Fast LOOCV/ALO_JRSSB/figures/time_linear_lasso_p_greater_n.pdf")
  } else {
    pdf("/Users/krad/Dropbox/Fast LOOCV/ALO_JRSSB/figures/time_linear_elnet_p_greater_n.pdf")
  }
} else {
  if (delta > 1){
    if (alpha_elnet == 1) {
      pdf( "/Users/krad/Dropbox/Fast LOOCV/ALO_JRSSB/figures/time_linear_lasso_p_less_n.pdf")
    } else {
      pdf( "/Users/krad/Dropbox/Fast LOOCV/ALO_JRSSB/figures/time_linear_elnet_p_less_n.pdf")
    }
  } else {
    if (alpha_elnet == 1) {
      pdf( "/Users/krad/Dropbox/Fast LOOCV/ALO_JRSSB/figures/time_linear_lasso_p_equal_n.pdf")
    } else {
      pdf( "/Users/krad/Dropbox/Fast LOOCV/ALO_JRSSB/figures/time_linear_elnet_p_equal_n.pdf")
    }
  }
}
